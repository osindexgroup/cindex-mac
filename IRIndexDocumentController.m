	...
	if ([typeName isEqualToString:CINArchiveType] || [typeName isEqualToString:CINXMLRecordType] ||
		...
		if (newndx)	{	// if have created new index
			if ([[IRIndexArchive alloc] initWithContentsOfURL:url ofType:typeName forIndex:[newndx iIndex]]) {
				if (!exists)	// if its a new index from imported records
					newndx.importSource = fileName.lastPathComponent;
				return (id)newndx;
			}
...
//		NSLog(@"%d, %@",tdoc, [tdoc displayName]);
		...
		if (tdoc.importSource) {	// if from import
			const char * impName = [tdoc.importSource UTF8String];
			infoSheet(tdoc.windowForSheet, INFO_NEWFROMIMPORT,tdoc.iIndex->head.rtot, impName);
			tdoc.importSource = nil;
		}
